# -*- coding: utf-8 -*-
"""
Тест: полная цепочка weather модуля

process_manager.enqueue_script → subprocess → weather_today_script → api_client → cache → graph → emit_event → bot_handler
"""
...
async def test_weather_full_chain_verbose():
    ...
    if len(received_events) == 0:
        ...
        print(f"   python -m scripts.weather.weather_today_script {lat} {lon} {user_id} test_task")
        print("   И проверь логи в logs/")
        return
    ...
    # === 5. ПРОВЕРКА КЭША ===
    print("\n🔍 5. Проверка кэша в local_db_weather...")
    from core.db.local_db_weather import get_cached_weather
    from datetime import datetime
    cached = get_cached_weather(lat, lon, datetime.now(), source="open_meteo")
    if cached:
